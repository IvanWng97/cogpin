...
ratchet eats its own dog food: this repo is gated by [`ratchet.toml`](ratchet.toml),
so the rules below aren't just etiquette — `ratchet stop` / the pre-push hook / the
...
(no npm/brew). One language-agnostic engine + one per-repo `ratchet.toml`. User-facing
...
5. **Base-pinning is bypass-proof.** The change layer reads `ratchet.toml` + the
...
- **TDD, always.** Failing test → minimal impl → green. Don't add a primitive without a
  ...
  check warns if you touch `ratchet.py` without touching `tests/`.
...
python3 ratchet.py validate                                 # the dogfood config
...
ruff check ratchet.py tests && mypy ratchet.py              # lint (config in pyproject.toml)
python3 ratchet.py check --cwd .                            # the self-gate (DoD on itself)
```
...
- Don't add a dependency or split `ratchet.py` — zero-deps/one-file is the product.
...
- Don't edit `ratchet.toml` / `ratchet.py` / the workflows to weaken a gate that's
