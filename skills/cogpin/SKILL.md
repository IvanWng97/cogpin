---
name: cogpin
description: Use when authoring or editing a cogpin.toml Definition-of-Done policy, or when a cogpin gate denies a command / blocks turn-end / reds CI — to fix the underlying cause properly instead of trying to bypass it.
---

# cogpin — authoring policies & responding to a block

cogpin is a Definition-of-Done gate. It enforces the closing-discipline an agent
...
| CI `check` is red | edit `cogpin.toml` to relax it | the config is base-pinned — relaxing it in the same PR does nothing; fix the change |
...
## Authoring a cogpin.toml

1. `python3 .cogpin/cogpin.py init` (or `/cogpin-init`) to scaffold, then edit.
...
3. Keep `base_pinned = true`. Add gate-defining files (`cogpin.toml`,
   `.cogpin/**`, CI workflows, hook configs) to a `protected_path` check — and a
   `self_protect` check for the real-time agent-layer twin.
4. `python3 .cogpin/cogpin.py validate` before committing.
...
Or skip the hand-authoring: `python3 .cogpin/cogpin.py suggest` reads the repo's
`CLAUDE.md` house-rules + structure into a ranked draft, and `/cogpin-init` walks the
whole flow. Write it to `cogpin.toml.draft` (never the live config), mark each
non-safe-core block with `# TODO(cogpin:review)`, run `draft-lint` until clean, then
the **human renames** `…draft → …toml` — that rename is the sign-off cogpin's whole
premise depends on (an auto-applied gate a human rubber-stamps is the exact corner-cut
it stops).

## Wiring & verifying

- `/cogpin-init` (→ `cogpin install`) wires the change layer once per repo: vendor
  `.cogpin/cogpin.py`, scaffold config, add the pre-push managed block + CI. It
  **coexists, never clobbers** — appends a sentinel-fenced block to the effective
  ...
  snippet instead of writing. Commit `.cogpin/cogpin.py`, `cogpin.toml`, and
  `.github/workflows/cogpin.yml`.
- `/cogpin-doctor` (→ `cogpin doctor`) confirms both layers are live, with a one-line
  fix for anything missing; `/cogpin-gaps` shows which house-rules are still prose.
- A teammate's local pre-push (CI already gates them):
  `python3 .cogpin/cogpin.py install --no-vendor --no-config --no-ci`.
...
`examples/{pixtuoid,node-ts,python,advisory}/cogpin.toml` for ready-to-lift policies.
