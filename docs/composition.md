# Composition map — where ratchet fits (and where it doesn't)

ratchet is **one layer** in a defense-in-depth stack, not the stack. It is the
**policy plane**: it declares which un-fabricable facts must hold before a change
merges, and renders a verdict. Every other layer is owned by something ratchet
*calls into* — never re-implements. A green ratchet gate is **not** a statement
that the work is good; reading it that way is the safety-theater failure this page
exists to prevent.

The "ratchet role" column below is the taxonomy: **policy plane** (ratchet owns it),
**capability floor** (ratchet *declares* a posture, the OS *enforces*), **verification
oracle** (ratchet *runs* the fact checks authoritatively, in CI), **review** (ratchet
fact-gates that review *happened*, never that it was good), **admission** (ratchet
hands the green signal off — out of scope).

## The stack

| Layer | Owner | Covers | ratchet's role |
|---|---|---|---|
| **Policy plane** (Definition-of-Done) | **ratchet** (`ratchet.toml` + `ratchet.py`) | Declaring which un-fabricable facts must hold before a change merges; binding `CLAUDE.md` house-rules to machine-checked facts | **policy plane** — ratchet *is* this layer; the single source of "done" |
| **Capability floor / sandbox** | OS + container + the agent harness (seccomp, Landlock, FS perms, network egress, the harness permission prompts) | What a process/agent *can do at all* — true containment | **capability floor** — ratchet *declares* the required posture and *emits* it to the harness (`[capability]` + `ratchet capability emit`); it **never contains**. The in-band command-deny is a forcing-function, not a sandbox |
| **Verification oracle** | CI runner (GitHub Actions / `action.yml`) over the base-pinned config; the test/lint/build/coverage tools it shells | Running the fact checks authoritatively on a machine the diff can't tamper with; running the actual suite | **verification oracle** — ratchet's authoritative run, base-pinned, fails *closed*. `run` / `require_checks_green` *delegate* to the real tools |
| **Review / approval** | Humans — PR reviewers, CODEOWNERS, forge branch-protection required reviews | Judgment: is the change correct / wise / good? | **review** — ratchet can require that review *happened* (`require_approval_from`, `approval_policy`, `pattern_requires_approval`; `marker_present` is a forcing-function nag) — never that it was *good* |
| **Admission / deploy** | Supply chain + admission control — Sigstore/cosign, SLSA provenance, Kyverno/OPA Gatekeeper, forge branch protection | Only signed/provenanced artifacts from a green pipeline reach prod; runtime admission | **admission** — **not covered.** ratchet gates the *change before merge*, not the *artifact at deploy*; it hands the green signal off |

## What ratchet does NOT cover

ratchet's claims are narrow on purpose. Five things it explicitly does **not** do —
...
2. **Code correctness / does-it-actually-work.** ratchet checks the facts you told
   it to; it does not know if the code is right. Whether the build passes and the
   tests are green is the **verification oracle**'s job — ratchet only *delegates*
   (`run`, `require_checks_green`) and reports the fact.
3. **Runtime containment.** ratchet cannot stop a process from doing anything. The
...
4. **Post-merge / deploy / runtime / production behavior.** Artifact signing, SLSA
   ...
   **admission** layer. ratchet stops at merge.
5. **Anything off the fact-surface.** Only `DiffFacts` / `CommandFacts` are visible.
   If a requirement isn't expressible as a fact over the diff or the command, ratchet
...
discipline was done *well*. ratchet's entire value lives in the gap between a *fact*
...
correct, was the review thoughtful, is the running system safe — ratchet contributes
...
it. ratchet raises the floor of what is *observable* before a merge; it does not raise
...
`ratchet.toml` *line* that delegates to a tool which already enforces it? A `run`
...
line — not by a new primitive.** ratchet is the policy plane that *composes* other
...
delegation line is dead weight in the engine and a claim ratchet now has to *own*
