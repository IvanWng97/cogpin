# Contributing to cogpin

Thanks for helping. cogpin has an unusually strong design spine — read this first so
...
advisory. `python3 cogpin.py validate` enforces this at parse time. A PR that tries to
make a judgment block won't merge — it would be a lie about the guarantee.

## Non-negotiable invariants

- **One file, zero runtime deps.** The engine is a single stdlib-only `cogpin.py`.
...
1. **TDD.** Add the failing test in `tests/test_cogpin.py` first — both a block path
...
python3 cogpin.py validate
for f in examples/*/cogpin.toml; do python3 cogpin.py validate --config "$f"; done
python3 scripts/validate_plugin.py
ruff check cogpin.py tests && mypy cogpin.py     # pip install ruff mypy
python3 cogpin.py check --cwd .                   # the self-gate (cogpin gates cogpin)
```
